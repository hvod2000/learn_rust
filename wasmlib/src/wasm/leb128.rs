...
impl<W: std::io::Write> Writer for W {
    ...
    fn f32(&mut self, value: f32) -> Result<()> { self.array(&value.to_le_bytes()) }
    fn f64(&mut self, value: f64) -> Result<()> { self.array(&value.to_le_bytes()) }
}
...
impl<R: std::io::Read> Reader for R {
    ...
    fn f32(&mut self) -> Result<f32> { Ok(f32::from_le_bytes(self.array()?)) }
    fn f64(&mut self) -> Result<f64> { Ok(f64::from_le_bytes(self.array()?)) }
	fn array<const N: usize>(&mut self) -> Result<[u8; N]> {
